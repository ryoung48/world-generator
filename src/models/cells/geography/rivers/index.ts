...
function addMeanderingPoints(start: Vertex, end: Vertex): Vertex[] {
  const deviationKm = window.dice.randint(5, 10)
...
export const RIVER = {
  length: (idx: number): number => {
    const river = window.world.rivers[idx]
    return river.cells.length + river.tributaries.reduce((acc, t) => acc + RIVER.length(t), 0)
  },
  parent: (idx: number): River => {
    const river = window.world.rivers[idx]
    if (river.branch !== -1) return RIVER.parent(river.branch)
    return window.world.rivers[idx]
  },
  remove: (idx: number) => {
    const river = window.world.rivers[idx]
    river.cells
    ...
    river.tributaries.forEach(RIVER.remove)
    delete window.world.rivers[idx]
  },
  spawn: {
    downstream: (src: Cell) => {
      if (src.river !== undefined) return
      const idx = Object.keys(window.world.rivers).length
      src.river = idx
      const river = [src.idx]
      ...
      let earlyExit = false
      let parent = -1
      const mountain = CELL.neighbors({ cell: src }).find(n => n.isMountains)?.mountain
      while (flowing && !branch && !earlyExit) {
        ...
        const potential = CELL.neighbors({ cell: curr }).filter(
          n =>
        ...
        if (selected.climate === 'arctic') {
          ...
          if (
            validBranch &&
            !selected.isCoast &&
            window.world.rivers[selected.river].mountain === mountain
          ) {
            branch = true
            parent = selected.river
          ...
          const lake = window.world.landmarks[selected.landmark].type === 'lake'
          if (lake) earlyExit = true
          else flowing = false
        } else {
          river.push(selected.idx)
      ...
      if (!earlyExit) {
        const segments = river.map(i => {
          ...
          return (
            cell.isCoast && coast.length > 0
              ? [coast[0].point.x, coast[0].point.y]
              : [cell.x, cell.y]
          ) as [number, number]
        })
        ...
        window.world.rivers[parent]?.tributaries.push(idx)
      } else {
        river
          .filter(i => window.world.cells[i].river === idx)
          .forEach(i => {
