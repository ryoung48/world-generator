...
const _weapon: Omit<WeaponDetails, 'tag' | 'weight' | 'markets'> = {
  ...
  value: function (item: Weapon) {
    ...
    return valueByQuality[item.tier] * mod
  }
}
...
const weaponTypes: string[] = [...weapon__types]
export const item__isWeapon = (item: Item): item is Weapon => {
  return weaponTypes.includes(item?.tag)
}
export const item__isTwoHandedWeapon = (item: Item) => {
  return item__isWeapon(item) && weapons[item.tag].handing === 'two-handed'
}
