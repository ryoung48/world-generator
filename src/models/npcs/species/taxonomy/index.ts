...
import { eyeColor, facialHair, hairStyles } from './common/appearance'
import { humanoids__demihumans } from './demihumans'
import { Humanoid, humanoid__species } from './types'

export const humanoid__speciesInit = (): Record<humanoid__species, Humanoid> => ({
...
export const humanoid__speciesDist = (count: number, tribal = true): humanoid__species[] => {
  const dist: WeightedDistribution<humanoid__species> = tribal
    ? [
        { v: 'human', w: 3 },
        { v: 'dwarf', w: 1 },
        { v: 'orc', w: 1 },
...
export const species__byCulture = (culture: Culture) => window.world.humanoids[culture.species]

export const species__finalizeCulture = (params: { culture: Culture; capital: Loc }) => {
