...
const religion__traits: Record<religion__tags, ReligionTrait> = {
  'ancestor worship': {
...
const { colors, spawn } = entity__traits({ traits: religion__traits, tag: 'religion' })

export const religion__spawnTraits = (religion: Religion) => {
  while (religion.traits.length < 3) {
    spawn({ entity: religion })
  }
}

export const religion__traitColors = colors
