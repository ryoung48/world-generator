...
import { actor__planChild } from '../events/planning'
...
export class Child implements Relation {
  ...
  public beforeSpawn(params: ActorParams) {
    ...
    params.socialClass = actor__socialClass({ actor: parent })
    if (!window.world.historyRecording) {
      params.socialClass = socialClass__randomDrift(params.socialClass)
    }
    params.birthLoc = birthLoc ?? window.world.locations[parent.location.curr]
    ...
    if (child) params.location = window.world.locations[parent.location.curr]
