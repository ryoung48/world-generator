...
const attributeGate =
  ...
  ({ actor }) =>
    actor.attributes[attr] >= floor ? 1 : 0.5
...
export const skillGate__wisdom = attributeGate({ attr: 'wisdom', floor: 10 })
...
export const skillGate__chaotic = personaLowGate({ attr: 'order', floor: 0.5 })
...
export const skillGate__nonPoor: ValidActorSkill = ({ actor }) =>
  profession__socialClass(actor.occupation.key) === 'lower' ? 0.1 : 1
