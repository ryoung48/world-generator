import { location__isVillage } from '../../../regions/locations/spawn/taxonomy/settlements'
...
export const backgrounds__court: Record<Court, Background> = {
  ...
  'blandished leadership': {
    ...
    type: 'court',
    context: `The leader has been led astray by {one|multiple} lovers. They dote on their paramours, granting them extravagant favors and imperiling the court with the lengths they go to in order to please their beloved. Those not given to romantic dizziness may be enraptured by more physical talents, so occupied in the business of night that nothing is done in the day.`,
    enemies: [
  ...
  'rival power': {
    tag: 'rival power',
    type: 'court',
    context: `The official hierarchy of the court is being challenged by a second power source within the court, one strong enough to stymie its official lord ({a powerful lord|a charismatic religious faction|an intrusive consul of a superior power|an impatient heir with too many friends}). Neither power source can act freely while the other exists, but destroying the rival may bring down the court in the process.`,
    enemies: [
...
const valid = (loc: Loc): WeightedDistribution<Thread['background']['court']> => {
  const village = location__isVillage(loc)
  ...
  return [
    ...
    {
      v: 'royal',
      w: province.capital ? 1 : 0
    }
  ]
}

export const courts__spawn = (loc: Loc) => {
  const backgrounds = loc.threads.map(i => window.world.threads[i].background.court)
  const courts = valid(loc).filter(court => !backgrounds.includes(court.v) && court.w > 0)
