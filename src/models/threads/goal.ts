...
import { actor__spawn } from './actors'
import { Actor } from './actors/types'
import { Background, BackgroundActor } from './backgrounds/types'
import { Thread } from './types'

const hooks = [
  'An Enemy has suffered a setback and seeks a Thing in order to rescue their situation.',
  'An Enemy is behind a recent Complication that is causing problems for the PCs.',
...
const decorateActor = (params: {
  tag: Background['context'][number]['tag']
  actor: BackgroundActor
  thread: Thread
  role: Actor['role']
  label: string
}) => {
  const { actor, role, thread, label, tag } = params
  const { idx } = actor__spawn({
    thread,
    role,
    profession: 'custom',
    age: actor.age ? window.dice.choice(actor.age) : undefined,
    gender: actor.gender,
    cast: 0
  })
  const npc = window.world.actors[idx]
  npc.profession.title = `${window.dice.spin(actor.alias).toLowerCase()} (${tag})`
  return decorateText({ label, link: npc })
}

export const thread__goal = (thread: Thread) => {
  const { context } = thread.background
  const hook = window.dice.choice(hooks)
  const [bgf, bge] = window.dice.shuffle(context)
  const friend = decorateActor({
    actor: bgf.friend,
    role: 'patron',
    thread,
    label: 'friend',
  ...
  const anotherEnemy = hook.includes('another Enemy')
    ? decorateActor({
        actor: bgf.enemy,
        role: 'rival',
        thread,
  ...
  const complication = `${bdc.complication} (${bdc.tag})`
  const thing = `${bgt.thing} (${bgt.tag})`
  const place = `${bgp.place} (${bgp.tag})`
  const alt = [bgf, bge].find(b => b !== bgp)
  const differentPlace = `${alt.place} (${alt.tag})`
  return window.dice.spin(
    hook
      .replaceAll('another Enemy', anotherEnemy)
