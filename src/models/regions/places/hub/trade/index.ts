...
export const TRADE_GOODS = {
  ...
  spawn: (hub: Hub) => {
    ...
    const biome = PLACE.climate(hub)
    const coastal = province.ocean > 0
    const mountains = province.mountains > 0
    const desert = biome.terrain === 'desert' || biome.terrain === 'glacier'
    const forest = biome.terrain === 'forest'
    const plains = biome.terrain === 'plains' || biome.terrain === 'tundra'
    const tropical = CLIMATE.zone[biome.latitude] === 'tropical'
    const arctic = CLIMATE.zone[biome.latitude] === 'arctic'
    const urban = HUB.city(hub)
    hub.trade = TRAIT.selection({
