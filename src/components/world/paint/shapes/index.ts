...
export const MAP_SHAPES = {
  ...
  color: {
    ...
    rebels: (opacity: number) => `rgba(88, 24, 13, ${opacity})`,
    routes: {
  ...
  camp: ({ ctx, scale, point }: DrawFeaturesParams) => {
    ...
    // Fill the tent interior with white
    ctx.beginPath()
    ...
    // Draw the tent outline
    ctx.strokeStyle = 'black'
    ctx.lineWidth = 0.015 * scale
  ...
  projection: {
    build: (ctx: CanvasRenderingContext2D) => MAP_SHAPES.projection.equirectangular(ctx),
