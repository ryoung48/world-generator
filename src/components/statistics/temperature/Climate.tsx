...
export const ClimateChart = () => {
  const raw = window.world.cells.filter(cell => cell.climate).map(cell => cell.climate)
  const climate = Object.entries(ARRAY.counter(raw)).sort((a, b) => b[1] - a[1])
  const labels = climate.map(rec => rec[0])
  const values = climate.map(rec => rec[1])
    ...
    datasets: [
      {
        ...
        backgroundColor: labels.map(
          k => MAP_METRICS.climate.categories[k as keyof typeof MAP_METRICS.climate.color]
        )
      }
    ]
  }
